...
async function initApp() {
    ...
    setupRefreshLogic();
    await refreshData();
    // 診斷放在主要資料流程完全跑完之後才執行，避免其中3個Twelve Data測試請求
    // 跟主流程搶同一組每分鐘8次的免費額度，導致真正要畫圖表的請求反而被限流
    runApiDiagnostics(); // 背景執行，不阻塞畫面，F12 Console 可看結果
}
...
//  驗證：2024合計=28943億✓ 2025合計=38091億✓ 2026/1-8累計=33868.7億✓（年增39.3%）
//  所有YoY與Yahoo Finance顯示值逐月核對一致
// ═══════════════════════════════════════════════════════════════
function renderMonthlyRevCharts() {
    // 各年月份資料（億元）— 來源：Yahoo Finance 仟元 ÷ 100000
    // 驗證：2022=22639億✓ 2023=21617億✓ 2024=28943億✓ 2025=38091億✓ 2026/1-8=33,868.7億✓（台積電官方公告，年增39.3%）
    ...
    const rev2026 = [4012.6,3176.6,4151.9,4107.3,4169.8,4426.8,4675.8,5148.06]; // 7月：4,675.8億，年增44.7%；8月：5,148.06億，月增10.1%、年增53.3%，單月歷史新高（台積電2026/9/10公告）
    ...
    const allLabels = [
        ...
        ...months.map(m=>`24/${m}`),
        ...months.map(m=>`25/${m}`),
        ...['01','02','03','04','05','06','07','08'].map(m=>`26/${m}`)
    ];
    ...
    // 近12個月 YoY 趨勢圖（2025/09 ~ 2026/08）
    const yoy12Labels = ['25/09','25/10','25/11','25/12','26/01','26/02','26/03','26/04','26/05','26/06','26/07','26/08'];
    const yoy12Data   = [31.4, 16.9, 24.5, 20.4, 36.8, 22.2, 45.2, 17.5, 30.1, 67.9, 44.7, 53.3];
